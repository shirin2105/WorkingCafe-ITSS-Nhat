    ...
    if (cafeGrid) {
        try {
            ...
            if (response.success && response.data && response.data.length > 0) {
                cafeGrid.innerHTML = ''; // Clear loading text
                ...
                response.data.forEach(cafe => {
                    ...
                    const cardHTML = `
                        <a href="cafe-detail.html?id=${cafe.id}" class="cafe-card">
...
const getCafeImages = (cafeId, fallbackUrl) => {
    const key = `cafe_images_${cafeId}`;
    let stored = [];
    try {
    ...
    const cleaned = Array.isArray(stored) ? stored.filter((url) => !!url) : [];
...
const saveCafeImages = (cafeId, images) => {
    const key = `cafe_images_${cafeId}`;
...
const applyLanguage = () => {
    ...
    if (document.body.classList.contains('search-body')) {
        ...
        if (locationBtn) locationBtn.textContent = dict.location;
        if (featureBtn) featureBtn.textContent = dict.feature;
        if (timeBtn) timeBtn.textContent = dict.time;
...
const loadNotifications = async (container) => {
    ...
    const response = await apiClient.get(`/notifications?user_id=${userId}&is_read=false`);
    ...
    const renderList = () => {
        container.innerHTML = notifications
            .map((item) => {
                ...
                return `
                    <button class="notification-item" data-id="${item.id}" data-booking-id="${bookingId || ''}">
                        ${item.content}
                    </button>
                `;
            })
    ...
    container.querySelectorAll('.notification-item').forEach((item) => {
        item.addEventListener('click', () => {
            ...
            const bookingId = item.dataset.bookingId || null;
            renderDetail(notificationId, bookingId);
        });
    });
};
...
async function initSearchPage() {
    ...
    const renderResults = () => {
        ...
        pageItems.forEach((cafe) => {
            const imageUrl = cafe.image_url || 'cafe.png';
            const openTime = cafe.open_time || '';
            const closeTime = cafe.close_time || '';
            const timeText = openTime && closeTime ? `${openTime}-${closeTime}` : '';
            const card = document.createElement('a');
            card.className = 'cafe-card';
            card.href = `cafe-detail.html?id=${cafe.id}`;
            card.innerHTML = `
                <img src="${imageUrl}" alt="${cafe.name}" class="cafe-card-img" onerror="this.src='cafe.png'">
                <div class="cafe-card-info">
                    <h3 class="cafe-card-title">${cafe.name}</h3>
    ...
    const renderTags = (keyword, selectedCity, selectedTime, selectedFeatures) => {
        ...
        const addTag = (label, type, value) => {
            ...
            tag.addEventListener('click', () => {
                if (type === 'keyword') keywordInput.value = '';
    ...
    document.addEventListener('click', (event) => {
        if (event.target.closest('.filter-group') || event.target.closest('.filter-pill-row')) {
            return;
        }
        closeGroups();
    });
...
async function initCafeDetailPage() {
    ...
    if (timeTextEl) {
        const open = cafe.open_time || '--:--';
        const close = cafe.close_time || '--:--';
        timeTextEl.textContent = `${open} - ${close}`;
    }
    let cafeImages = getCafeImages(cafeId, cafe.image_url);
            ...
            galleryEdit.addEventListener('click', async () => {
                await openGalleryManager({
                    ...
                    onSave: async (nextImages) => {
                        saveCafeImages(cafeId, nextImages);
...
async function initCafeMapPage() {
    ...
    const updateSidebar = async (cafe) => {
        ...
        if (timeEl) timeEl.textContent = `${cafe.open_time || '--:--'} - ${cafe.close_time || '--:--'}`;
        if (ratingEl) ratingEl.innerHTML = `<i class="fa-solid fa-star"></i> 0/5`;
    ...
    if (cafeId) {
        const target = cafes.find((entry) => String(entry.id) === String(cafeId));
        if (target && !coordsByCafeId.has(Number(target.id))) {
            const geocoded = await geocodeAddress(target.address, target.city);
            if (geocoded) {
                coordsByCafeId.set(Number(target.id), geocoded);
            }
        }
    }

    if (typeof L === 'undefined' || !mapFrame) {
        await updateSidebar(cafes[0]);
        return;
    }
    ...
    mapInstance = L.map(mapFrame).setView(
        center ? [center.lat, center.lng] : pickDefaultCenter(cafes, coordsByCafeId),
        14
    );
    ...
    if (canEditLocation) {
        activeCafe = targetCafe;
        mapInstance.on('click', async (event) => {
            const { lat, lng } = event.latlng || {};
            if (!Number.isFinite(lat) || !Number.isFinite(lng)) return;
            const confirmed = await openConfirmModal('Chọn vị trí', 'Cập nhật vị trí quán tại đây?');
            if (!confirmed) return;
            const response = await apiClient.put(`/cafes/${activeCafe.id}`, {
                latitude: lat,
                longitude: lng
            });
            if (!response.success) {
                await openAlertModal('Lỗi', response.error || 'Không thể cập nhật vị trí quán.');
                return;
            }
            activeCafe.latitude = lat;
            activeCafe.longitude = lng;
            coordsByCafeId.set(Number(activeCafe.id), { lat, lng });
            let marker = markerByCafeId.get(Number(activeCafe.id));
            if (!marker) {
                marker = L.marker([lat, lng]).addTo(mapInstance);
                marker.on('click', () => {
                    window.location.href = `cafe-detail.html?id=${activeCafe.id}`;
                });
                markerByCafeId.set(Number(activeCafe.id), marker);
            } else {
